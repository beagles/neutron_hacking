...
class LbaasAgentApi(object):
    ...
    def __init__(self, topic, context, host):
        super(LbaasAgentApi, self).__init__()
    ...
    def get_ready_devices(self):
        return self.client.call(self.context, 'get_ready_devices',
                                host=self.host)

    def pool_destroyed(self, pool_id):
        return self.client.call(self.context, 'pool_destroyed',
                                pool_id=pool_id)

    def pool_deployed(self, pool_id):
        return self.client.call(self.context, 'pool_deployed',
                                pool_id=pool_id)

    def get_logical_device(self, pool_id):
        return self.client.call(self.context, 'get_logical_device',
                                pool_id=pool_id)

    def update_status(self, obj_type, obj_id, status):
        return self.client.call(self.context, 'update_status',
                                obj_type=obj_type,
                                obj_id=obj_id,
                                status=status)

    def plug_vip_port(self, port_id):
        return self.client.call(self.context, 'plug_vip_port',
                                port_id=port_id,
                                host=self.host)

    def unplug_vip_port(self, port_id):
        return self.client.call(self.context, 'unplug_vip_port',
                                port_id=port_id,
                                host=self.host)

    def update_pool_stats(self, pool_id, stats):
        return self.client.call(self.context, 'update_pool_stats',
                                pool_id=pool_id,
                                stats=stats,
                                host=self.host)
